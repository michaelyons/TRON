...
module.exports = class Game {
  ...
  trailCollision() {
    ...
    for(let i=this.trail1.length-2 ; i>-1; i--) {
      if(this.trail1[this.trail1.length-1] === this.trail1[i]) {
        this.player1.x = this.player1.x - 2
        this.player1.y = this.player2.y - 2
    ...
    // this.trail1.forEach((bike1, i) => {
    //   this.trail1.forEach(function (bike2, j) {
  ...
  wallCollision() {
    if (this.player1.x > canvas.width || this.player1.x < 0) {
      this.player1.x = this.trail1[0][1].x
      this.player1.y = this.trail1[0][1].y
      this.player1.dx = 0
      this.player1.dy = 0
      this.player2.dx = 0
      this.player2.dy = 0
